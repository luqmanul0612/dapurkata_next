    ...
    case "GET": {
      ...
      if (bookId || slug) {
        const book = await prisma.book.findUnique({
          where: { id: bookId as string, slug: slug as string },
          include: { Image: { select: { url: true, secureUrl: true } } },
        });
        if (book) res.status(200).json({ statusCode: "200", data: book });
        else
          res
            .status(404)
            .json({ statusCode: "404", message: "Book not found" });
      } else {
      ...
      auth(req, res)
        .then(async () => {
          try {
            ...
            if (bufferFile.byteLength > limit)
              throw new Error(
                JSON.stringify({
                  statusCode: "400",
                  message: `Max file size ${limit / 1024}`,
                })
              );
            else {
              const image = await cloudinary.uploader.upload(req.body.cover, {
                folder: "dapurkata/books",
              });
    ...
    case "PUT": {
      auth(req, res).then(async () => {
        try {
          ...
          if (req.body.cover) {
            saveFile({
              dir: "/public/uploads/books",
              file: req.body.cover,
              name: `${new Date().getTime()}-${findBook?.title}` as string,
              limit: 1048576,
          ...
          const updateBook = await prisma.book.update({
            ...
            data: {
              ...
              isbn: req.body.isbn || undefined,
              slug: stringPath(
                `${req.body.publisher || findBook?.publisher}-${
                  req.body.title || findBook?.title
                }`
              ),
            },
          });
